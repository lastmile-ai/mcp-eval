...
session = await TestSession.create(
    ...
    server_configs=my_server_configs
)
try:
    agent = await session.start()
    # Your test code
finally:
...
session = await TestSession.create(
    ...
    timeout_seconds=300,
    output_dir="test-reports/custom"
)
```
...
full_response = await agent.generate("Explain quantum computing")
print(full_response.content)  # The actual text
print(full_response.model)    # Which model was used
print(full_response.usage)    # Token usage stats
```
