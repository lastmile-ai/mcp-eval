...
# Exact match
await session.assert_that(
    Expect.content.equals("The answer is 42"),
    response=response
)
...
await session.assert_that(
    Expect.tools.output_matches(
        ...
        match_type="contains"  # or "equals", "regex"
    )
)
```
...
await session.assert_that(
    Expect.path.efficiency(
        ...
        allow_extra_steps=0,  # No unnecessary steps
        penalize_backtracking=True,  # Fail if agent goes backward
...
await session.assert_that(
    Expect.path.includes_waypoints(
        required_tools=["auth", "fetch", "logout"],
        max_distance_between=3,  # Up to 3 steps between waypoints
        allow_duplicates=False
    )
)

# Check path doesn't include dangerous patterns
await session.assert_that(
    Expect.path.avoids_patterns(
        forbidden_sequences=[
            ["delete", "confirm"],  # Prevent accidental deletion
...
<CardGroup cols={2}>
  ...
  <Card title="Custom Evaluators" href="./custom-evaluators" icon="code">
