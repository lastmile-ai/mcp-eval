...
@task(
    name="Complex test",
    description="Tests advanced features with retries",
    tags=["integration", "slow"],
    timeout_seconds=300,
    max_retries=3
)
async def test_complex(agent, session):
    # Your test code
    pass
```
...
@task(
    ...
    config={
        ...
        "max_tokens": 4000  # Longer responses allowed
    }
)
...
### Parametrize with named scenarios

```python
from mcp_eval.core import parametrize_scenario

scenarios = [
    {"name": "simple", "url": "https://example.com", "expected": "Example Domain"},
    {"name": "secure", "url": "https://secure.example.com", "expected": "Secure"},
    {"name": "api", "url": "https://api.example.com/v1", "expected": "API"}
]

@with_agent("default")
@parametrize_scenario(scenarios)
@task("Test URL fetching scenarios")
async def test_urls(agent, name, url, expected):
    response = await agent.generate_str(f"Fetch {url}")
    
    await agent.assert_that(
        Expect.content.contains(expected),
        name=f"has_{name}_content"
    )
```
...
```python
from mcp_eval.core import on_test_complete

@on_test_complete
def log_test_result(result: TestResult):
    """Custom logging after each test."""
...
```python
import os
from mcp_eval.core import task, skip_if, only_if

@skip_if(os.getenv("CI") == "true", reason="Skip in CI")
@task("Local-only test")
async def test_local_only(agent):
    # This test only runs locally
    pass

@only_if(os.getenv("RUN_SLOW_TESTS") == "true")
@task("Expensive test")
async def test_expensive(agent):
...
```python
from mcp_eval.core import task, depends_on

@task("Setup database")
async def test_setup_db(agent):
    response = await agent.generate_str("Create test database")
    await agent.assert_that(Expect.content.contains("created"))

@depends_on(test_setup_db)
@task("Test with database")
async def test_with_db(agent):
    # This only runs if test_setup_db passes
    response = await agent.generate_str("Query test database")
```
...
async def test_error_handling(agent):
    ...
    await agent.assert_that(
        Expect.tools.no_errors(),
        name="no_tool_crashes"
    )
```
