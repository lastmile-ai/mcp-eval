...
Use `mcp-eval server add` or import from `mcp.json` / DXT. See [Quickstart](./quickstart.mdx) and [CLI Reference](./cli-reference.mdx).
...
`mcp-eval run tests/test_file.py::test_func` or `mcp-eval run dataset --file path.yaml`.

### Where are configs discovered?

[config.py](../src/mcp_eval/config.py) documents discovery for `mcpeval.yaml` and secrets.
