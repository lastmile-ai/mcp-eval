...
@task(description="A simple success case for getting the time in a major city.", server="sample_server")
async def test_get_time_in_london(agent, session):
    ...
    tool_arguments_match("get_current_time", {"timezone": "Europe/London"}, session=session)

    # Confirm the overall goal was met
    objective_succeeded(objective, response, session=session)

@task(description="A test designed to FAIL by checking summarization quality.", server="sample_server")
...
@task(description="Tests if the agent can chain tools to achieve a multi-step objective.", server="sample_server")
...
@task(description="Tests how the agent handles a known error from a tool.", server="sample_server")
