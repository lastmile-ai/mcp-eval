...
from mcp_eval import (
    task, setup, parametrize, test_session,
    ToolWasCalled, ResponseContains, LLMJudge, Case, Dataset
)
...
async def test_enhanced_judge(agent, session):
    ...
    response = await agent.generate_str("Fetch https://example.com and explain what it is")
...
async def test_span_analysis(agent, session):
    ...
    if span_tree:
        ...
        if rephrasing_loops:
            session._record_evaluation_result("no_rephrasing_loops", False, 
        ...
        for analysis in path_analyses:
            session._record_evaluation_result("path_efficiency", 
...
async def dataset_with_enhanced_features():
    ...
    for result in report.results:
        if result.span_tree:
            ...
            if loops:
                print(f"Warning: Found {len(loops)} rephrasing loops in {result.case_name}")
            
            recovery_sequences = result.span_tree.get_error_recovery_sequences()
            if recovery_sequences:
                successful_recoveries = sum(1 for seq in recovery_sequences if seq.recovery_successful)
                print(f"Error recovery: {successful_recoveries}/{len(recovery_sequences)} successful")
