                ...
                try:
                    async with session as test_agent:
                        ...
                        return CaseResult(
                            ...
                            metrics=session.get_metrics(),
                            passed=all(
                                isinstance(r, (int, float)) and r > 0.5 or
                                isinstance(r, dict) and r.get('score', 0) > 0.5 or
                                r is True
            ...
            'cases': [
                {
                    ...
                    'metadata': case.metadata,
                    'evaluators': [
                        {type(e).__name__: e.to_dict()}
                        for e in case.evaluators
                    ]
                }
...
async def generate_test_cases(
    ...
    return await generator.generate_cases(
        ...
        categories=categories or ['basic', 'error_handling', 'performance', 'edge_cases'],
    )