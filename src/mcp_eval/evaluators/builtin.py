...
class MaxIterations(SyncEvaluator):
    ...
    def evaluate_sync(self, ctx: EvaluatorContext) -> Dict[str, Any]:
        ...
        return {
            ...
            'actual': actual,
            'score': 1.0 if passed else max(0.0, 1.0 - (actual - self.max_iterations) / self.max_iterations)
        }
...
class ToolSuccessRate(SyncEvaluator):
    ...
    def evaluate_sync(self, ctx: EvaluatorContext) -> Dict[str, Any]:
        ...
        if not tool_calls:
            return {'passed': True, 'rate': 1.0, 'score': 1.0}  # No calls = perfect success
    ...
    def evaluate_sync(self, ctx: EvaluatorContext) -> Dict[str, Any]:
        ...
        if self.exact_match:
            ...
            if isinstance(ctx.output, str) and isinstance(ctx.expected_output, str):
                ...
                expected = ctx.expected_output.lower() if not self.case_sensitive else ctx.expected_output
