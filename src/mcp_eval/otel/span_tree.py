...
class SpanTree:
    ...
    def get_llm_rephrasing_loops(self, max_similar_calls: int = 3, similarity_threshold: float = 0.8) -> List[LLMRephrasingLoop]:
        ...
        while i < len(llm_calls):
            ...
            while j < len(llm_calls):
                ...
                if self._prompts_similar(base_prompt, current_prompt, similarity_threshold):
            ...
            if len(similar_group) >= max_similar_calls:
                total_duration = (similar_group[-1].end_time - similar_group[0].start_time)
                
                loops.append(LLMRephrasingLoop(
                    spans=similar_group,
                    loop_count=len(similar_group),
                    total_duration=total_duration,
                    is_stuck=len(similar_group) > max_similar_calls * 1.5  # Very stuck if much longer
                ))
    ...
    def get_inefficient_tool_paths(self, golden_paths: Dict[str, List[str]]) -> List[ToolPathAnalysis]:
        ...
        for task_name, golden_path in golden_paths.items():
            if self._path_matches_task(actual_path, task_name):
                efficiency_score = self._calculate_path_efficiency(actual_path, golden_path)
                unnecessary_calls = self._find_unnecessary_calls(actual_path, golden_path)
                missing_calls = self._find_missing_calls(actual_path, golden_path)
                
                analyses.append(ToolPathAnalysis(
    ...
    def get_error_recovery_sequences(self) -> List[ErrorRecoverySequence]:
        ...
        for error_span in error_spans:
            ...
            for span in self.all_spans():
                if (span.start_time > error_span.end_time and 
                    span.start_time < error_span.end_time + timedelta(minutes=5)):  # Within 5 minutes
                    recovery_spans.append(span)
            
            if recovery_spans:
                # Check if recovery was successful (no more errors in recovery spans)
                recovery_successful = not any(span.has_error() for span in recovery_spans)
                recovery_duration = (recovery_spans[-1].end_time - recovery_spans[0].start_time)
                
                recovery_sequences.append(ErrorRecoverySequence(
    ...
    def _extract_prompt(self, llm_span: SpanNode) -> str:
        ...
        prompt = (
            llm_span.attributes.get('gen_ai.prompt', '') or
            llm_span.attributes.get('llm.prompt', '') or
            llm_span.attributes.get('input', '') or
            llm_span.name
        )
    ...
    def _calculate_path_efficiency(self, actual_path: List[str], golden_path: List[str]) -> float:
    ...
    def _find_unnecessary_calls(self, actual_path: List[str], golden_path: List[str]) -> List[str]:
    ...
    def _find_missing_calls(self, actual_path: List[str], golden_path: List[str]) -> List[str]:
