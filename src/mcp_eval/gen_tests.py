...
class TestCase(BaseModel):
    test_name: str = Field(description="A valid Python function name for the test, e.g., 'test_search_for_weather'.")
    description: str = Field(description="A human-readable description for the @task decorator.")
    objective: str = Field(description="The natural language prompt to send to the agent to execute the test.")
    assertions: List[str] = Field(description="A list of keywords that should be present in the response for a basic 'contains' assertion.")
...
async def list_tools_for_server(server_name: str) -> str:
    ...
    try:
        ...
        temp_agent = Agent(name="mcpeval-test-generator", server_names=[server_name], context=temp_app.context)
        await temp_agent.initialize()
        tools_result = await temp_agent.list_tools()
        
        for tool in tools_result.tools:
            if tool.name == "__human_input__": continue
...
async def generate_tests_from_llm(server_name: str, tool_descriptions: str) -> GeneratedTests:
    ...
    llm_provider_name = next((p.name for p in app.config.llm_providers if p.provider == "openai"), None)
...
@app.command()
def generate(
    server_name: str = typer.Argument(..., help="The name of the MCP server to generate tests for."),
    output_file: str = typer.Option(None, "--output", "-o", help="The path to save the generated test file."),
):
    """
    ...
    if not tool_descriptions:
        console.print("[bold red]No tools found or server is unavailable. Cannot generate tests.[/bold red]")
        raise typer.Exit(1)
        
    console.print("[green]Found tools...[/green] Asking LLM to generate test cases...")
    
    try:
        generated_tests = asyncio.run(generate_tests_from_llm(server_name, tool_descriptions))
    except Exception as e:
    ...
    if output_file:
        ...
        syntax = Syntax(output_code, "python", theme="solarized-dark", line_numbers=True)
