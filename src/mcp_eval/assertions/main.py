"""Legacy assertion functions - maintained for backwards compatibility."""
...
from mcp_eval.evaluators.builtin import (
    ToolWasCalled, ResponseContains, ToolSuccessRate, LLMJudge, 
    MaxIterations, ToolSequence
)
...
def _get_session():
    ...
    if not hasattr(_local, 'session'):
        raise RuntimeError("No active test session. Use @task decorator or with test_session().")
...
def contains(response: str, text: str, case_sensitive: bool = False):
...
def not_contains(response: str, text: str, case_sensitive: bool = False):
...
def matches_regex(response: str, pattern: str):
    """Assert that response matches regex pattern."""
    session = _get_session()
    evaluator = ResponseContains(text=pattern, regex=True)
    session.evaluate_now(evaluator, response, f"matches_regex")


def tool_was_called(tool_name: str, min_times: int = 1):
...
def tool_was_called_with(tool_name: str, arguments: dict):
...
def tool_call_count(tool_name: str, expected_count: int):
    ...
    session.add_deferred_evaluator(evaluator, f"tool_count_{tool_name}_{expected_count}")


def tool_call_succeeded(tool_name: str):
...
def tool_call_failed(tool_name: str):
...
def tool_success_rate(min_rate: float, tool_name: Optional[str] = None):
...
def completed_within(max_iterations: int):
    """Assert task completed within max iterations."""
    session = _get_session()
...
async def judge(response: str, rubric: str, min_score: float = 0.8):
