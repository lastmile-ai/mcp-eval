...
class JudgeResult(BaseModel):
    score: float = Field(ge=0.0, le=1.0, description="A score from 0.0 to 1.0 on how well the response meets the criteria.")
    reasoning: str = Field(description="A brief explanation for the score.")

class ObjectiveSuccessResult(BaseModel):
    succeeded: bool = Field(description="A boolean flag, true if the response successfully accomplishes the objective, false otherwise.")
    reasoning: str = Field(description="A brief explanation for the decision.")

class EfficiencyResult(BaseModel):
    is_efficient: bool = Field(description="A boolean flag, true if the plan was efficient, false if there were significant redundancies or better paths.")
    reasoning: str = Field(description="An explanation of any inefficiencies found or a confirmation of efficiency.")
    suggested_improvement: Optional[str] = Field(None, description="A more efficient plan of action, if applicable.")
...
def tool_was_called(tool_name: str, min_times: int = 1, session: TestSession | None = None):
    ...
    def check(metrics: TestMetrics):
        ...
        assert tool_metric["call_count"] >= min_times, f"Tool '{tool_name}' was called {tool_metric['call_count']} times, expected at least {min_times}."
...
def tool_success_rate_is_above(tool_name: str, min_rate: float, session: TestSession):
    ...
    def check(metrics: TestMetrics):
        tool_metric = metrics.tool_metrics.get(tool_name)
        assert tool_metric is not None, f"Tool '{tool_name}' was not called, so success rate cannot be calculated."
        
        call_count = tool_metric.get('call_count', 0)
        assert call_count > 0, f"Tool '{tool_name}' was called 0 times, so success rate cannot be calculated."
        
        success_count = tool_metric.get('success_count', 0)
        success_rate = success_count / call_count
        assert success_rate >= min_rate, f"Tool '{tool_name}' success rate {success_rate:.2f} is below the minimum of {min_rate:.2f}."
...
def tool_arguments_match(tool_name: str, expected_args: dict, session: TestSession):
    ...
    def check(metrics: TestMetrics):
        tool_call = next((tc for tc in metrics.tool_calls if tc.name == tool_name), None)
        assert tool_call is not None, f"Tool '{tool_name}' was not called."
        
        for key, expected_value in expected_args.items():
            assert key in tool_call.arguments, f"Tool '{tool_name}' was called without argument '{key}'."
            actual_value = tool_call.arguments[key]
            assert actual_value == expected_value, f"Argument '{key}' for tool '{tool_name}' did not match. Expected '{expected_value}', got '{actual_value}'."
...
def number_of_steps_under(max_steps: int, session: TestSession):
    ...
    def check(metrics: TestMetrics):
        assert metrics.turns <= max_steps, f"Agent took {metrics.turns} steps, which exceeds the limit of {max_steps}."
...
def cost_under(max_cost: float, session: TestSession):
    ...
    def check(metrics: TestMetrics):
        ...
        assert total_cost < max_cost, f"Total cost ${total_cost:.6f} exceeded the limit of ${max_cost:.6f}"

    check.__name__ = f"cost_under(${max_cost:.6f})"
    session.add_deferred_assertion(check)

def llm_judge(response_to_judge: str, rubric: str, min_score: float, session: TestSession):
    ...
    async def check(_: TestMetrics):
        ...
        judge_result = await session.agent.llm.generate_structured(prompt, response_model=JudgeResult)
        assert judge_result.score >= min_score, f"Judge score {judge_result.score} is below the minimum of {min_score}. Reasoning: {judge_result.reasoning}"
...
def objective_succeeded(objective: str, final_response: str, session: TestSession):
    ...
    async def check(_: TestMetrics):
        ...
        """
        judge_result = await session.agent.llm.generate_structured(prompt, response_model=ObjectiveSuccessResult)
        assert judge_result.succeeded, f"Objective was not met. Reasoning: {judge_result.reasoning}"
...
def plan_is_efficient(objective: str, session: TestSession):
    ...
    async def check(metrics: TestMetrics):
        ...
        judge_result = await session.agent.llm.generate_structured(prompt, response_model=EfficiencyResult)
        assert judge_result.is_efficient, f"Plan was deemed inefficient. Reasoning: {judge_result.reasoning}"
