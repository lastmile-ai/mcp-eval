...
def print_failure_details(console: Console, failed_results: List[TestResult], verbose: bool = False, captured_logs: Optional[Dict[str, str]] = None) -> None:
...
def _print_verbose_failure_details(console: Console, result: TestResult, log_output: Optional[str] = None) -> None:
    ...
    if hasattr(result, '_agent_details') and 'instruction' in result._agent_details:
        ...
        if len(instruction) > max_length:
            ...
            console.print(f"  [dim](truncated, {len(instruction)} total characters)[/dim]")
        else:
    ...
    if result.parameters:
        ...
        for key, value in result.parameters.items():
            ...
            if isinstance(value, dict):
                ...
                console.print(f"  {key}: [{', '.join(str(v) for v in value[:3])}, ... ({len(value)} items)]")
            else:
        ...
        for eval_record in result.evaluation_results:
            if not eval_record.passed:
                ...
                if eval_record.result:
                    ...
                    if hasattr(eval_record.result, 'message') and eval_record.result.message:
    ...
    if result.metrics:
        ...
        if isinstance(result.metrics, dict):
            ...
            if 'total_duration_ms' in result.metrics:
                console.print(f"  Total Duration: {result.metrics['total_duration_ms']:.2f}ms")
    ...
    if log_output and log_output.strip():
        ...
        console.print(pad(" Captured stdout call ", "-", console=console, length=80), style="dim")
...
def _print_artifact_links(console: Console, test_name: str) -> None:
    ...
    if trace_file.exists():
        files_found.append(("Trace (JSONL)", trace_file, "View detailed execution trace"))
    if metrics_file.exists():
        files_found.append(("Metrics (JSON)", metrics_file, "View test metrics and results"))
