...
class MetricsConfig(BaseSettings):
    ...
    collect: List[str] = Field(default_factory=lambda: [
        "response_time", "tool_coverage", "iteration_count", "token_usage", "cost_estimate"
    ])
    token_prices: Dict[str, Dict[str, float]] = Field(default_factory=lambda: {
...
def get_current_config() -> Dict[str, Any]:
    ...
    return {
        ...
        'agent_config': _current_settings.agent_config or _current_settings.agents['default'].dict(),
        'servers': {name: config.dict() for name, config in _current_settings.servers.items()},
