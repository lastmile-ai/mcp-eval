...
def process_spans(spans: List[TraceSpan]) -> TestMetrics:
    ...
    metrics.success_rate = 1.0 - (len(error_calls) / len(tool_calls)) if tool_calls else 1.0
...
def _is_tool_call_span(span: TraceSpan) -> bool:
    ...
    return (
        'tool' in span.name.lower() or
        'call_tool' in span.name or
...
def _is_llm_span(span: TraceSpan) -> bool:
    ...
    return (
        span.attributes.get('gen_ai.system') is not None or
        'llm' in span.name.lower() or
        'generate' in span.name.lower()
    )
...
def _extract_tool_call(span: TraceSpan) -> Optional[ToolCall]:
    ...
    try:
        tool_name = (
            span.attributes.get('mcp.tool.name') or
            span.attributes.get('tool.name') or
            span.name.replace('call_tool_', '').replace('tool_', '')
        )
        ...
        is_error = (
            ...
            span.attributes.get('mcp.tool.error') is not None or
            any(event.get('level') == 'error' for event in span.events)
        )
